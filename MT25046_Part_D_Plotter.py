...
def plot_part_c():
    ...
    # ✅ Added "Time(s)" here
    for metric in ["CPU%", "Mem(MB)", "IO(MB/s)", "Time(s)"]:
        plt.figure()
        ...
        plt.savefig(
            os.path.join(
                OUTDIR,
                f"part_c_{metric.replace('%','pct').replace('(','').replace(')','').replace('/','')}.png"
            ),
            dpi=300
        )
        plt.close()
    ...
    for worker in ["cpu", "mem", "io"]:
        ...
        for metric in ["CPU%", "Mem(MB)", "IO(MB/s)", "Time(s)"]:
            ...
            plt.savefig(
                os.path.join(
                    OUTDIR,
                    f"part_d_{metric.replace('%','pct').replace('(','').replace(')','').replace('/','')}_{worker}.png"
                ),
                dpi=300
            )
            plt.close()
